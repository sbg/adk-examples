...
def load_manifest(filename):
    ...
    def parse_manifest_into_cohort(filename):
        ...
        logging.info("  %d manifest entries read." % num_entries)  # will equals total number of samples * number of lanes
    ...
    def stage_input_files_in_bulk(cohort):
        ...
        files_to_stage = [f for s in cohort.samples 
            for l in s.lanes for f in [l.fq1, l.fq2]]
                    
        staged_files = FindOrCopyFilesByName("StageInputs",
        ...
        for sample in cohort.samples:
            for lane in sample.lanes:
                ...
                metadata_records.extend([{
                     'sample_id' : sample.id, 
                     'file_segment_number' : lane.read_group
                 }] * 2)
    
        SetMetadataBulk(to_files=files_to_update, 
                        metadata=metadata_records, keep_old=True,
                        name_="SetMetadata")
